...
export function OperationPanel(props: Props) {
  ...
  return (
    <View className="rounded-3xl bg-white p-4 dark:bg-[#142235]">
      <Text className="text-xs font-bold uppercase tracking-[2px] text-slate-400">Operaciones</Text>
      ...
      <Text className="mt-5 text-xs font-bold uppercase tracking-wider text-slate-500">Cuenta origen</Text>
      ...
      <Text className="mt-4 text-xs font-bold uppercase tracking-wider text-slate-500">Monto en USD</Text>
      ...
      {props.mode === "transfer" ? (
        <>
          <Text className="mt-4 text-xs font-bold uppercase tracking-wider text-slate-500">Tipo de destino</Text>
          ...
          <Text className="mt-4 text-xs font-bold uppercase tracking-wider text-slate-500">Cuenta destino</Text>
          {isExternal ? (
          ...
          {isExternal ? (
            <>
              <Text className="mt-2 text-xs text-slate-500">Validaremos la cuenta antes de mover fondos.</Text>
              ...
              {!props.mcpPinConfigured ? <Text className="mt-2 text-xs text-amber-700">Configura tu PIN en Ajustes antes de confirmar.</Text> : null}
            </>
          ) : null}
          {!destinations.length && !isExternal ? <Text className="mt-2 text-xs text-slate-500">Abre otra cuenta desde Cuentas para transferir entre tus cuentas.</Text> : null}
        </>
      ) : null}

      <Text className="mt-4 rounded-2xl bg-slate-50 p-4 text-xs leading-5 text-slate-500">{props.mode === "deposit" ? "Agrega fondos a la cuenta que elijas." : props.mode === "withdrawal" ? "Retira solo fondos disponibles." : "Revisa origen, destino y monto antes de confirmar."}</Text>
      {props.mcpActionPending ? <Text className="mt-3 rounded-xl bg-blue-50 p-3 text-sm text-blue-800">Finaliza o cancela la operación pendiente desde el asistente antes de iniciar otra.</Text> : null}
      {props.notice ? <Text className="mt-3 rounded-xl bg-amber-50 p-3 text-sm text-amber-800">{props.notice}</Text> : null}
